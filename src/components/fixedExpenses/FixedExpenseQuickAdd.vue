<!-- components/QuickAdd.vue -->
<template>
    <div class="col-lg-4">
      <div
        class="card shadow-sm p-4 h-100 d-flex align-items-center justify-content-center"
        style="cursor: pointer;"
        @click="isOpen = true"
      >
        <h6 class="text-muted mb-2">빠른 추가</h6>
        <div
          class="border rounded w-100 d-flex align-items-center justify-content-center"
          style="height: 100px;"
        >
          <span class="fs-2 text-muted">+</span>
        </div>
      </div>
  
      <!-- 모달 컴포넌트 -->
      <QuickAddModal v-model="isOpen" />
    </div>
  </template>
  
  <script setup>
  import { ref } from 'vue'
  import QuickAddModal from '../modal/FixedExpenseAddModal.vue'
  
  const isOpen = ref(false)
  </script>
  